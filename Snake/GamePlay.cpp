...
void GamePlay::ProcessInput()
{
    ...
    while (m_context->m_window->pollEvent(event))
    {
        ...
        else if (event.type == sf::Event::KeyPressed)
        {
            ...
            if (std::abs(m_snakeDirection.x) != std::abs(newDirection.x) ||//180 isu
...
void GamePlay::Update(sf::Time deltaTime)
{
    if (!m_isPaused)
    {
        ...
        {
            for (auto& wall : m_walls)//koliziya stenok
            {
            ...
            if (m_snake.IsOn(m_food))//food eat
            {
...
void GamePlay::Draw()
{
    ...
    //m_context->m_window->draw(m_poison);
